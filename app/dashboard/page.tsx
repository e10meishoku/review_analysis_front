...
import { DashboardBenefitsChart } from "@/components/dashboard/benefits-chart" // 修正対象
import { DashboardIssuesChart } from "@/components/dashboard/issues-chart" // 修正対象
...
export default async function DashboardPage({ searchParams }: DashboardPageProps) {
  ...
  // ▼▼▼ 追加: Step 5で追加したデータを取得 ▼▼▼
  const benefitsData = dashboardData?.benefits || []
  const issuesData = dashboardData?.issues || []

  // 年代別の結合処理
  const combinedAgeData = (distData?.age || []).map((ageDistItem) => {
    const ratingItem = ageRatingData.find(r => r.label === ageDistItem.label)
    return {
      label: ageDistItem.label,
      count: ageDistItem.count,
      rating: ratingItem?.count || 0 
    }
  })
      ...
      */}
      ...
      <div className={`grid grid-cols-1 xl:grid-cols-2 gap-8 ${rowHeight}`}>
        ...
        <div className="bg-primary rounded-[2rem] p-6 shadow-sm h-full flex flex-col relative overflow-hidden">
             ...
             <div className="flex-1 min-h-0 w-full">
                 {/* ▼ データを渡す */}
                 <DashboardIssuesChart data={issuesData} />
             </div>
        </div>
      </div>
